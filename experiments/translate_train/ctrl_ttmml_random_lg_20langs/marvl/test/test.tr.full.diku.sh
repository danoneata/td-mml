#!/bin/bash
#SBATCH --job-name=trfull_mar_random_lg
#SBATCH --ntasks=1 --cpus-per-task=20
#SBATCH -p gpu --gres=gpu:titanrtx:1
#SBATCH --time=2:00:00
#SBATCH --output="logs/multilingual.marvl-tr.random_lg.step_100000.batch_size_256.langs_20.eval_step2.full.out"
...
TRTASK=marvl-${STRATEGY}-batch_size_${TRAIN_BATCH_SIZE}-step_${STEP}-langs_${LANGS}-full
TETASK=MaRVL${LANG}-${STRATEGY}-batch_size_${TRAIN_BATCH_SIZE}-step_${STEP}-langs_${LANGS}-eval_step_${EVAL_STEP}-full
