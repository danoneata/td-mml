...
# add functions for ttml
from ttml.utils.misc import parse_with_config
from ttml.utils.logger import RunningMeter
from ttml.data import (DetectFeatLmdb,MlmDataset,mlm_collate,MlmDataset_Multilingual,mlm_multilingual_collate,
                       ConcatDatasetWithLens,TokenBucketSampler,PrefetchLoader,MetaLoader)
...
def parse_args():
    ...
    parser.add_argument('--config_ttml_paths', required=True, help='JSON config files')
...
def build_mlm_dataset(img_db,is_train, args, config,tokenizer, blind):

    if blind:
        collate_fn = mlm_multilingual_collate
        datasets = MlmDataset_Multilingual(img_db,is_train, args, config,tokenizer, task_name="mlm-multilingual")
    else:
        collate_fn = mlm_collate
        datasets = MlmDataset(img_db,is_train, args, config,tokenizer, task_name="mlm")
    dataset = ConcatDatasetWithLens(datasets)

    return dataset, collate_fn

def create_dataloaders(args, is_train, config, tokenizer):
    dataloaders = {}
    num_dataset=0
    if is_train:
        path = os.path.join(args.features_path,'training_feat.lmdb')
        img_db = DetectFeatLmdb(path,args,config)
    else:
        path = os.path.join(args.features_path,'validation_feat.lmdb')
        img_db = DetectFeatLmdb(path,args,config)

    for i, t in enumerate(args.task.split(",")):
        task = f'{t}'
        if task.startswith('mlm'):
            blind = 'multilingual' in task
            dataset = build_mlm_dataset(img_db,is_train, args, config,tokenizer, blind)
            num_dataset = len(dataset)
        # elif task.startswith('itm'):
        #     dataset = build_itm_dataset(img_db,is_train, args, config,tokenizer)
        else:
        ...
        #     loader = build_dataloader_itm(*dataset, is_train, args)
        # else:
        #     loader = build_dataloader(*dataset, is_train, args)
        loader = build_dataloader(*dataset, is_train, args)
        if is_train:
            ratio = args.mix_ratio.split(',')[i]
            dataloaders[task] = (loader, ratio)
        else:
            dataloaders[task] = PrefetchLoader(loader)
    return dataloaders,num_dataset
...
def main():
    ...
    train_dataloaders, train_num_dataset= create_dataloaders(
        args, True, config, tokenizer)
    val_dataloaders, val_num_dataset = create_dataloaders(
        args, False, config,tokenizer)
    ...
    task_ids = [''.join(['TASK',str(id)]) for id, m in enumerate(task_names)]
    # task_names = ["Conceptual_Caption"]

    # task_ids = ["TASK0"]
    # task2num_iters = {"TASK0": train_num_dataset / args.train_batch_size}
    ...
    if default_gpu:
        ...
        logger.info("  Num examples = %d", train_num_dataset)
    ...
    for epoch_id in range(start_epoch, int(args.num_train_epochs)):
        for step, (name,batch) in enumerate(meta_loader):
            ...
            iter_id = start_iter_id + step + (epoch_id * train_num_dataset)
