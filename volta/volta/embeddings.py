...
class UniterEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.token_type_embeddings_2_dims = nn.Embedding(2,config.hidden_size)
        self.token_type_embeddings_2_dims.weight.data.normal_(mean=0.0, std=self.initializer_range)
    ...
    def forward(self, token_ids, image_feat, image_loc, token_type_ids=None, position_ids=None, sequence_b_segment_id=1):
        batch_size, num_boxes, _ = image_feat.shape
        seq_length = token_ids.size(1)
        ...
        if position_ids is not None and self.model == "bert-base-uncased":
            position_ids = torch.arange(seq_length, dtype=torch.long, device=token_ids.device)
            position_ids = position_ids.unsqueeze(0).expand_as(token_ids)
        ...
        words_embeddings = self.word_embeddings(token_ids)
        position_embeddings = self.position_embeddings(position_ids)
        if sequence_b_segment_id in token_type_ids:
            token_type_embeddings = self.token_type_embeddings_2_dims(token_type_ids)
        else:
            token_type_embeddings = self.token_type_embeddings(token_type_ids)
