...
def main():
    ...
    train_vtlm_dataset = train_vtlm_dataset.__iter__()
    # Train
            ...
            if step % args.grad_acc_steps*2 == 0 and step >0:
                for _ in range(args.grad_acc_steps):
                    ...
                    if vtlm_batch is not None:
                        vtlm_batch = tuple(t.cuda(device=device, non_blocking=True) for t in vtlm_batch[:-1])
                        input_ids, input_mask, segment_ids,position_ids, lm_label_ids, \
                        ...
                        mlm_loss, _, _ = model(input_ids=input_ids, image_feat=image_feat, image_loc=image_loc,
                                                       ...
                                                       obj_labels = obj_labels, obj_confs = obj_confs,
