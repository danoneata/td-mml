...
class ImageFeaturesH5Reader_cc(ImageFeaturesH5Reader):
    def __init__(self, features_path: str, config, in_memory: bool = False):
        self.features_path = features_path
        self._in_memory = in_memory
        ...
        # If not loaded in memory, then list of None.
        if 'mwp141' in self.features_path:
            lmdb_file = os.path.join(self.features_path, "validation_feat.lmdb", "validation_feat_all.lmdb")
        ...
        with self.env.begin(write=False) as txn:
            self._image_ids = list(txn.cursor().iternext(values=False))
    ...
    def __getitem__(self, image_id):
        ...
        if self._in_memory:
            ...
            with self.env.begin(write=False) as txn:
                ...
                try:
                    features = np.frombuffer(base64.b64decode(item["features"]), dtype=np.float32).reshape(-1,
                                                                                                           self.feature_size)
                    boxes = np.frombuffer(base64.b64decode(item['boxes']), dtype=np.float32).reshape(-1, 4)
                except:
                    features = item["features"].reshape(-1, self.feature_size)
